...
namespace StarterAssets
{
    ...
    {
        [Header("Player")]
        public GameObject playerMainGO;

        [Tooltip("Is the player locked in place?")]
        public bool playerPositionLocked = false;
        ...
        [SerializeField] private bool playerOnWalkableSurface;
        [SerializeField] private bool playerSwimming;
        ...
        private void GroundedCheck()
        {
            ...
            if (Physics.Raycast(transform.position, Vector3.down, out rayHit, rayDistance, GroundLayers, QueryTriggerInteraction.Ignore))
            {
                ...
                else if (!rayHit.transform.CompareTag("Ship") || rayHit.transform.CompareTag("Structure"))
                {
                    ...
                    gameObject.transform.SetParent(playerMainGO.transform);
                    playerOnWalkableSurface = false;
                }
            }
        ...
        //This function can be used to determine fall damage begin later
        /*private IEnumerator SetAirborneStatus()
        {
        ...
        Crest.SampleHeightHelper _waterHeight = new Crest.SampleHeightHelper();
        private bool onSurfaceOfWater;
        //To do: Implement the ocean sample helper information that keeps the player on the surface
        private void SwimmableCheck()
        {
            /*if (transform.position.y <= 5 && !Grounded)
            {
                if (!transform.parent.CompareTag("Ship"))
                {
                    Debug.Log("Player position below swim line");
                    _waterHeight.Init(transform.position, 2f * transform.lossyScale.magnitude);
                    onSurfaceOfWater = true;

                }
            }
            if (transform.position.y >= 0 && !Grounded)
            {
                if (!transform.parent.CompareTag("Ship"))
                {
                    _waterHeight.Init(transform.position, 2f * transform.lossyScale.magnitude);
            ...
            {
                onSurfaceOfWater = false;
            }*/

            if (onSurfaceOfWater)
            {
                _waterHeight.Init(transform.position, 2f * transform.lossyScale.magnitude);
                if (_waterHeight.Sample(out var height))
                {
        ...
        private bool playerSwimmingBelowSurface;
        public void PlayerSwimCheck()
        {
            if (!Grounded)
            {
                onSurfaceOfWater = true;
        ...
        //Lack of input causes character speed to stay at 0.
        //Move needs to be called until the character is grounded again
        private void Move()
        {
